...
tf.flags.DEFINE_float('max_epsilon', 8, 'max epsilon.')

tf.flags.DEFINE_integer('num_iter', 4, 'max iteration.')
...
tf.flags.DEFINE_string('input_dir', r'E:\OneDrive - hust.edu.cn\files\自然天气图像攻击\图片\图1 渲染图\添加了恶劣天气\haze',
                       'Input directory with images.')

tf.flags.DEFINE_string('output_dir', r'E:\OneDrive - hust.edu.cn\files\自然天气图像攻击\图片\图1 渲染图\添加了恶劣天气\attack_haze2',
...
def batch_grad(x, one_hot, i, max_iter, alpha, grad):
    ...
    with slim.arg_scope(resnet_v2.resnet_arg_scope()):
        logits_v3, end_points_v3 = resnet_v2.resnet_v2(
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=config) as sess:
            s1.restore(sess, model_checkpoint_map['resnet_v2'])
            idx = 0
            l2_diff = 0
